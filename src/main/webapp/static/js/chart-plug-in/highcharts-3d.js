...
(function(d) {
	function n(c, a, b) {
		var e, f, g = a.options.chart.options3d,
			i = !1;
	...
	function F(c) {
		if (this.chart.is3d()) {
			...
			if (a !== void 0 && !a && this.group.zIndex !== void 0 && !this.zIndexSet) this.group.attr({
	...
	d.SVGRenderer.prototype.cuboid = function(c) {
		...
		a.animate = function(a, c, f) {
			o(a.x) && o(a.y) ? (a = this.renderer.cuboidPath(a), this.front.attr({
				zIndex: a[3]
			}).animate({
					d: a[0]
				},
				...
				d: a[2]
			}, c, f)) : a.opacity ? (this.front.animate(a, c, f), this.top.animate(a, c, f), this.side.animate(a, c, f)) : d.SVGElement.prototype.animate.call(this, a, c, f);
			return this
		};
		a.destroy = function() {
			this.front.destroy();
	...
	d.SVGRenderer.prototype.cuboidPath = function(c) {
		var a = c.x,
			...
			h = n(h, d.charts[this.chartIndex], c.insidePlotArea),
			b = function(a, b) {
				a = j(a, function(a) {
					return h[a]
				});
	...
	d.SVGRenderer.prototype.arc3d = function(c) {
		...
		var a = this.g(),
			...
			f = b.zTop * 100;
		a.shapeArgs = c;
		a.top = e.path(b.top).setRadialReference(c.center).attr({
			zIndex: b.zTop
		}).add(a);
		a.side1 = e.path(b.side2).attr({
			zIndex: b.zSide1
		});
		a.side2 = e.path(b.side1).attr({
			zIndex: b.zSide2
		});
		a.inn = e.path(b.inn).attr({
			zIndex: b.zInn
		});
		a.out = e.path(b.out).attr({
			zIndex: b.zOut
		});
		a.fillSetter = function(a) {
			this.color = a;
	...
	d.SVGRenderer.prototype.arc3dPath = function(c) {
		...
		var w = j * l(k),
			o = j * l(h),
			j = ["M", a + s * v, b + t * p],
			j = j.concat(r(a, b, s, t, e, d, 0, 0)),
			j = j.concat(["L", a + u * c, b + i * n]),
			j = j.concat(r(a, b, u, i, d, e, 0, 0)),
			j = j.concat(["Z"]),
			k = k > 0 ? q / 2 : 0,
			h = h > 0 ? 0 : q / 2,
			k = e > -k ? e : d > -k ? -k : e,
			x = d < q - h ? d : e < q - h ? q - h : d,
			h = ["M", a + s * m(k), b + t * l(k)],
			h = h.concat(r(a,
				b, s, t, k, x, 0, 0)),
			h = h.concat(["L", a + s * m(x) + w, b + t * l(x) + o]),
			h = h.concat(r(a, b, s, t, x, k, w, o)),
			h = h.concat(["Z"]),
			k = ["M", a + u * v, b + i * p],
			k = k.concat(r(a, b, u, i, e, d, 0, 0)),
			k = k.concat(["L", a + u * m(d) + w, b + i * l(d) + o]),
			k = k.concat(r(a, b, u, i, d, e, w, o)),
			k = k.concat(["Z"]),
			v = ["M", a + s * v, b + t * p, "L", a + s * v + w, b + t * p + o, "L", a + u * v + w, b + i * p + o, "L", a + u * v, b + i * p, "Z"],
			a = ["M", a + s * c, b + t * n, "L", a + s * c + w, b + t * n + o, "L", a + u * c + w, b + i * n + o, "L", a + u * c, b + i * n, "Z"],
			b = l((e + d) / 2),
			e = l(e),
			d = l(d);
		return {
	...
	d.wrap(d.Chart.prototype, "renderSeries", function(c) {
		...
		if (this.is3d())
			for (; a--;) c = this.series[a], c.translate(), c.render();
		else c.call(this)
	});
	...
	d.wrap(d.Axis.prototype, "getPlotLinePath", function(c) {
		...
		var b = this.chart.options.chart.options3d,
			b = this.isZAxis ? this.chart.plotWidth : b.depth,
			d = this.opposite;
		this.horiz && (d = !d);
		a = [this.swapZ({
			x: a[1],
	...
	d.extend(D.prototype, {
		...
		getSeriesExtremes: function() {
			...
			d.each(c.series, function(b) {
				if (b.visible || !a.options.chart.ignoreHiddenSeries)
					if (c.hasVisibleSeries = !0, b = b.zData, b.length) c.dataMin = Math.min(y(c.dataMin, b[0]), Math.min.apply(null, b)), c.dataMax = Math.max(y(c.dataMax, b[0]), Math.max.apply(null, b))
			})
		}
	});
		...
		function(c) {
			...
			if (this.chart.is3d()) {
				var a = this.chart,
					b = this.options,
					e = b.depth || 25,
					f = (b.stacking ? b.stack || 0 : this._i) * (e + (b.groupZPadding || 1));
				b.grouping !== !1 && (f = 0);
				f += b.groupZPadding || 1;
	...
	d.wrap(d.seriesTypes.column.prototype, "init", function(c) {
		...
		if (this.chart.is3d()) {
			...
			if (b === void 0 || b) {
				...
				d = a.stack || 0;
				for (f = 0; f < b[d].series.length; f++)
					if (b[d].series[f] === this) break;
	...
	d.wrap(d.seriesTypes.pie.prototype, "translate", function(c) {
		...
		if (this.chart.is3d()) {
			var a = this,
				...
				j = g.alpha,
				h = g.beta,
				k = e.stacking ? (e.stack || 0) * f : a._i * f;
			k += f / 2;
			e.grouping !== !1 && (k = 0);
		...
		if (this.chart.is3d()) {
			...
			d.each(a.data, function(b) {
				var c = b.shapeArgs,
					d = c.r,
					g = c.depth,
					i = (c.alpha || a.chart.options.chart.options3d.alpha) * x,
					c = (c.start + c.end) / 2,
					b = b.labelPos;
	...
	d.wrap(d.seriesTypes.scatter.prototype, "translate", function(c) {
		...
		if (this.chart.is3d()) {
			var a = this.chart,
				b = d.pick(this.zAxis, a.options.zAxis[0]),
				e = [],
				f, g;
