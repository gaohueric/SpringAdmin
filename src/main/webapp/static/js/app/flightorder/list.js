...
$(document).ready(function(){
    ...
    vm_list=new Vue({
        ...
        methods:{
            // 加载list页面数据
            loadGridData:function(orderResultVO){
                $.ajax({
                    type:"POST",
                    url:__ctx + "/order/getOrderList",
                    data:orderResultVO,
                ...
                alertify.confirm("确定删除吗？", function(result) {
                    if (result) {
                        $.ajax({
                            ...
                            url : __ctx + '/order/deleteOrderInfo',
                            data : JSON.stringify({id : flightOrder.id})
                        }).done(function(resp) {
                            if (resp.result) {
                                toastr.success('删除成功');
                            } else {
