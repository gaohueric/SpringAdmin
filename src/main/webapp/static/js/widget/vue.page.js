...
Vue.component('vue-page', {
    ...
    '<ul class="pagination pull-right no-margin">' +
        '<li v-if="totalPage > 1 && currentPage != 1" >' +
            '<a v-on:click="firstPage ,pageClick($event)">首页</a>' +
        '</li>' +
        ...
        '<li v-else class="disabled">' +
            '<a>下一页</a>' +
        '</li>' +
        '<li v-if="currentPage < totalPage && totalPage > 1" >' +
            '<a v-on:click="lastPage ,pageClick($event)">尾页</a>' +
        '</li>' +
        '<li v-else class="disabled">' +
            '<a >尾页</a>' +
        '</li>' +
        ...
        '</li>' +
        '<li v-if="currentPage >= 1" >' +
            '<select v-on:change="optChange($event)" class="paginate_button col-sm-1" title = "页面大小">' +
            '<option>10</option>' +
                '<option>20</option>' +
                '<option>50</option>' +
                '<option>100</option>' +
    ...
    computed: {
        ...
        lastPage:function(){
            ...
            this.paginator.currentPage = this.paginator.totalPage;
            return this.paginator.currentPage;
        },
        totalPage: function() {
            if (!this.paginator) {
        ...
        getTargetPage:function(){
            ...
            if(targetPage<1 || targetPage>this.paginator.totalPage){
                return this.paginator.currentPage;
            }else{
                this.paginator.currentPage=targetPage;
                return this.paginator.currentPage;
            }
        },
        totalCount: function() {
